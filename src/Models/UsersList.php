...
class UsersList extends AbstractModel
{
  ...
  public function find($user_id, $date = null): array {
    ...
    $user_games = $this->database_connection->fetchAll(Game::class, passed_sql: "SELECT g.*, sum(ua.achieved) unlocked_achievements, count(a.id) total_achievements, sum(ua.achieved) / count(a.id) * 100 achievement_percent
                  FROM games g
